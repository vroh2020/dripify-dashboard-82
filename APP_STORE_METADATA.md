# 📱 App Store Metadata for Dripify AI

## App Information

### App Name
**Dripify AI**

### Subtitle (30 characters max)
AI-Powered Style Analysis

### Keywords (100 characters max)
**style,fashion,outfit,analysis,AI,clothing,rating,drip,wardrobe,fashionista**

### App Description (4,000 characters max)
```
Transform your style with AI-powered outfit analysis! Dripify AI uses advanced artificial intelligence to analyze your photos and provide personalized style ratings, detailed breakdowns, and actionable fashion tips.

Whether you're getting ready for a date, a night out, or just want to improve your daily look, Dripify AI is your personal style coach.

**Key Features:**
- **AI Style Score:** Get an objective score on your outfit's style.
- **Detailed Analysis:** Understand what works and what doesn't, from color coordination to fit.
- **Personalized Tips:** Receive actionable advice to instantly improve your look.
- **Track Your Progress:** See how your style evolves over time.
- **Inspiration:** Discover new styles and trends.

Download Dripify AI today and elevate your style game with AI-powered fashion analysis!

---

**Privacy & Terms:**
Your privacy is our priority. We use Apple Sign-In for secure authentication and process your photos locally for analysis. View our complete privacy policy and terms of service at https://dripcheck.framer.website

### Support URL
**https://dripcheck.framer.website**

### Marketing URL (Optional)
**https://dripcheck.framer.website**

## App Store Review Information

### Review Notes
```
Dear App Review Team,

Thank you for reviewing Dripify AI! Here's what you need to know:

**App Functionality:**
- Dripify AI is an AI-powered style analysis app
- Users upload outfit photos and receive AI-generated style ratings
- The app uses Apple Sign-In for authentication
- All photo processing is done securely through our AI service

**Testing Instructions:**
1. Download and open the app
2. Sign in with Apple (you can use a test Apple ID)
3. Complete the onboarding flow (age, goal selection)
4. Upload a photo of an outfit (you can use any clothing photo)
5. View the AI analysis results and style tips
6. Navigate through the dashboard to see all features

**Key Features to Test:**
- Apple Sign-In authentication
- Photo upload and analysis
- Style rating display
- Tips and recommendations
- Dashboard functionality
- Premium subscription flow (RevenueCat integration)

**Test Account (if needed):**
Email: test@dripcheck.app
Password: [Use Apple Sign-In]

**Technical Notes:**
- Built with React + Capacitor
- Uses Supabase for backend services
- RevenueCat for subscription management
- Nebius AI for style analysis
- iOS 14+ compatible

**Privacy Compliance:**
- Privacy policy: https://dripcheck.framer.website/privacy-policy
- Terms of service: https://dripcheck.framer.website/terms-of-services
- Apple Sign-In for secure authentication
- No data collection beyond what's necessary for app functionality

If you have any questions or need additional information, please don't hesitate to contact us.

Best regards,
The Dripify AI Team
```

### Demo Account (if required)
**Email:** test@dripcheck.app  
**Password:** Use Apple Sign-In with test account

### Contact Information
**Name:** Dripify AI Support  
**Email:** ramcharanvelpuri@gmail.com  
**Phone:** [Your support phone number]

## Content Rating

### Age Rating
**4+ (Everyone)**

### Content Descriptors
- **None** - App contains no objectionable content
- **No Violence** - No violent content
- **No Sexual Content** - No sexual content
- **No Language** - No profanity
- **No Substances** - No drug/alcohol references

### Category
**Primary:** Photo & Video  
**Secondary:** Lifestyle

## Screenshots Required

### iPhone Screenshots (Required)
You need screenshots for these device sizes:

1. **6.7" iPhone (iPhone 14 Pro Max, 15 Pro Max)**
   - Screenshot 1: Welcome/Onboarding screen
   - Screenshot 2: Photo upload interface
   - Screenshot 3: Style analysis results
   - Screenshot 4: Dashboard with stats
   - Screenshot 5: Tips and recommendations

2. **6.5" iPhone (iPhone 11 Pro Max, 12 Pro Max, 13 Pro Max)**
   - Same screenshots as above

3. **5.5" iPhone (iPhone 8 Plus, SE 2nd gen)**
   - Same screenshots as above

### Screenshot Guidelines
- **Resolution:** Use actual device screenshots
- **Content:** Show real app functionality
- **Quality:** High resolution, clear text
- **No overlays:** Don't add text or graphics over screenshots
- **No mockups:** Use actual app screenshots

## App Preview Video (Optional but Recommended)

### Video Requirements
- **Duration:** 15-30 seconds
- **Format:** MP4
- **Resolution:** 1920x1080 or higher
- **Content:** Show app functionality in action

### Video Content Ideas
1. **Quick Demo:** Show photo upload → analysis → results
2. **Feature Highlight:** Focus on AI analysis capabilities
3. **User Journey:** Complete onboarding to dashboard flow

## Pricing & Availability

### Pricing
- **Free:** Basic features with limited analyses
- **Premium:** $12.99/month or $99.99/year
- **Free Trial:** 7-day free trial for premium features

### Availability
- **Countries:** All App Store countries
- **Languages:** English
- **Age Rating:** 4+

## Keywords for App Store Optimization

### Primary Keywords
- style analysis
- fashion app
- outfit rating
- AI fashion
- style tips
- fashion advice
- wardrobe app
- style checker

### Long-tail Keywords
- AI style analysis app
- outfit rating app
- fashion advice app
- style tips app
- wardrobe management app
- fashion rating app
- style checker app
- AI fashion assistant

## App Store Connect Checklist

### ✅ Completed
- [x] App bundle uploaded to TestFlight
- [x] Privacy policy and terms of service URLs
- [x] App icon and launch screen
- [x] Code signing and provisioning
- [x] Apple Sign-In integration
- [x] Camera and photo library permissions

### ❌ Still Needed
- [ ] App description and keywords
- [ ] Screenshots for all device sizes
- [ ] App preview video (optional)
- [ ] Review notes and demo account
- [ ] Content rating configuration
- [ ] Pricing and availability settings
- [ ] App Store Connect metadata submission

## Next Steps

1. **Create Screenshots:** Take screenshots on actual devices
2. **Write Description:** Use the provided description above
3. **Set Keywords:** Use the provided keywords
4. **Configure Rating:** Set content rating to 4+
5. **Add Review Notes:** Use the provided review notes
6. **Submit for Review:** Complete App Store Connect setup

## Contact Information

For App Store review questions:
- **Email:** ramcharanvelpuri@gmail.com
- **Website:** https://dripcheck.framer.website
- **Privacy Policy:** https://dripcheck.framer.website/privacy-policy
- **Terms of Service:** https://dripcheck.framer.website/terms-of-services 