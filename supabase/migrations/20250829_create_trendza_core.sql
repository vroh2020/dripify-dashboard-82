-- Trendza core schema: color analysis, closet, and product search
...
CREATE TABLE IF NOT EXISTS public.trendza_color_profiles (
  ...
  palette JSONB NOT NULL DEFAULT '[]'::jsonb, -- array of hex strings and metadata
  face_metrics JSONB NOT NULL DEFAULT '{}'::jsonb, -- extracted measurements/colors
...
CREATE TABLE IF NOT EXISTS public.trendza_closet_items (
  ...
  color TEXT,
  season TEXT,
  tags JSONB NOT NULL DEFAULT '[]'::jsonb, -- auto tags from CV
  attributes JSONB NOT NULL DEFAULT '{}'::jsonb, -- size, fit, material
  created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(),
  updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW()
);
...
ALTER TABLE public.trendza_products ENABLE ROW LEVEL SECURITY; -- readable via policy if needed
...
-- Products: default deny; later add read policy if we cache catalog per user/partner
