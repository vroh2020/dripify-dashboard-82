...
try {
  ...
  execSync(`/usr/libexec/PlistBuddy -c "Add :buildSettings:PROVISIONING_PROFILE_SPECIFIER string Dripify AI" ${projectPath}/project.pbxproj || true`, { stdio: 'inherit' });
  execSync(`xcodebuild -project ${projectPath} -scheme App -configuration Debug PROVISIONING_PROFILE_SPECIFIER="Dripify AI" CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY="Apple Distribution: Velpuri Enterprises Inc. (TN748MMP9M)" DEVELOPMENT_TEAM=TN748MMP9M build -showBuildSettings || true`, { stdio: 'inherit' });
  execSync(`xcodebuild -project ${projectPath} -scheme App -configuration Release PROVISIONING_PROFILE_SPECIFIER="Dripify AI" CODE_SIGN_STYLE=Manual CODE_SIGN_IDENTITY="Apple Distribution: Velpuri Enterprises Inc. (TN748MMP9M)" DEVELOPMENT_TEAM=TN748MMP9M build -showBuildSettings || true`, { stdio: 'inherit' });
} catch (error) {
