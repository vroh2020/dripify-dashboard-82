...
import { Apple, Star, Mail } from "lucide-react";
import { handleAppleSignIn, handleContinueWithEmail, handleGoogleSignIn } from "../utils/auth";
...
export const WelcomeStep = ({ onNext }: WelcomeStepProps) => {
  const handleGoogleClick = async () => {
    ...
    if (success) {
      // If Google auth succeeds, it will redirect automatically
      // If it fails, we still continue with onboarding
      onNext();
    }
  };
  ...
  const handleAppleClick = async () => {
    ...
    if (success) {
      // If Apple auth succeeds, it will redirect automatically
      // If it fails, we still continue with onboarding
      onNext();
    }
  };
  ...
  return (
    ...
    >
      ...
      {/* Button Area - Fixed at bottom, always visible */}
      <div className="flex-shrink-0 space-y-3 px-6 pb-6">
        <Button
          onClick={handleGoogleClick}
          className="w-full bg-white hover:bg-gray-100 text-black h-14 text-base font-semibold rounded-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center shadow-xl"
        >
        ...
        <Button
          ...
          className="w-full bg-black hover:bg-gray-900 text-white h-14 text-base font-semibold rounded-2xl transition-all duration-300 hover:scale-105 flex items-center justify-center shadow-xl"
        >
        ...
        <Button
          ...
          className="w-full bg-white/10 border-2 border-white/20 text-white hover:bg-white/20 hover:border-white/30 h-14 text-base font-semibold rounded-2xl transition-all duration-300 hover:scale-105 backdrop-blur-sm"
        >
