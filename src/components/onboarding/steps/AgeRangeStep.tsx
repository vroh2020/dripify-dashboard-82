...
export const AgeRangeStep = ({ onNext, onBack }: AgeRangeStepProps) => {
    ...
    >
        ...
        {AGE_RANGES.map((ageRange, index) => (
          <motion.button
            ...
            className={`w-full h-[58px] rounded-2xl text-[17px] font-semibold transition-all duration-200 
              ${selectedAgeRange === ageRange ? "bg-black text-white" : "bg-[#F7F7FB] text-black hover:bg-gray-100"}`}
          >
            {ageRange}
          </motion.button>
        ))}
      ...
      >
        <motion.button
          ...
          whileTap={selectedAgeRange ? { scale: 0.98 } : {}}
          onClick={() => {
            if (selectedAgeRange) {
              onNext(selectedAgeRange);
            }
          }}
          className={`w-full h-[56px] rounded-2xl text-[17px] font-semibold transition-all duration-200 
            ${selectedAgeRange ? "bg-black text-white hover:bg-gray-900" : "bg-gray-300 text-white cursor-not-allowed"}`}
        >
          Continue
        </motion.button>
      </motion.div>
